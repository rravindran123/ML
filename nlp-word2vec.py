...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  # print("Center vector shape")
  # print(center_word_vec.shape)
  # print(center_word_vec)
  #print(outside_vectors.shape)
  #print(grad_center_vec.shape)
  #print(grad_center_vec)
  #print(grad_outside_vecs)

  # Vocab size and embedding dimension
  vocab_size = outside_vectors.shape[0]
  embedding_dim = outside_vectors.shape[1]

  # Compute the scores (unnormalized log probabilities) by dot product of outside vectors and the center word vector
  scores = outside_vectors.dot(center_word_vec)  # Shape (vocab_size,)

  # Apply softmax to the scores to get probabilities
  probs = softmax(scores)  # Shape (vocab_size,)
  
  # Compute the loss (negative log probability of the correct outside word)
  loss = -np.log(probs[outside_word_idx])

  # Compute the gradient w.r.t. the center word vector (grad_center_vec)
  # The correct outside word contributes to the gradient
  probs[outside_word_idx] -= 1  # Subtract 1 from the probability of the correct word

  # grad_center_vec is the weighted sum of outside vectors, weighted by the softmax probabilities
  grad_center_vec = probs.reshape(1, vocab_size).dot(outside_vectors).flatten()  # Shape (embedding_dim,)

  # Compute the gradient w.r.t. all the outside word vectors (grad_outside_vecs)
  # Multiply probabilities (including the adjusted prob for the correct word) by the center word vector
  grad_outside_vecs = probs.reshape(vocab_size, 1).dot(center_word_vec.reshape(1, embedding_dim))  # Shape (vocab_size, embedding_dim)
...
def skipgram(current_center_word, window_size, outside_words, 
             word2ind, center_word_vectors, outside_vectors, dataset, 
  ...
  for index, outside_word in enumerate(outside_words):
     outside_index = word2ind[outside_word]
     loss_current, gradc, grado = word2vec_loss_and_gradient(center_word_vec=center_word_vectors[center_word_index],
                                                             outside_word_idx=outside_index,
