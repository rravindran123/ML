...
for _ in range(num_iterations):
   # print("input_ids size: " + str(input_ids.size()))
    ...
    with torch.no_grad():
        ...
        outputs = outputs = model(input_ids=input_ids,
    ...
    #print("Next Token is:", tokenizer.decode(generated_token_id[0], skip_special_tokens=True))
...
print(f"Total time take : {time.time() - total_time:.4f} seconds")
...
plt.title("LLM Generation Times for each token (with KV Caching)")
plt.savefig("/Users/raviravindran/Documents/Code/ML/ML/inferencing/latency-with-caching.png", dpi=300, bbox_inches="tight")
plt.show()
